        ...
        if($slug=="personal"){
            ...
            if($request->isMethod('post')){
                ...
                if($request->hasFile('address_proof_image')){
                    ...
                    $imageName = time() . '.' . $image->getClientOriginalExtension();
                    // Define la ruta donde se guardará la imagen
                    $imagePath = public_path('admin/images/proofs');
                ...
                if($vendorCount>0){
                    //Actualizar tabla dedetalles business
                    VendorsBusinessDetail::where('vendor_id',Auth::guard('admin')->user()->vendor_id)->update(['nombre_tienda'=>$data['nombre_tienda']
                    ,'celular_tienda'=>$data['celular_tienda'],'direccion_tienda'=>$data['direccion_tienda'],'ciudad_tienda'=>$data['ciudad_tienda'],'estado_tienda'=>$data['estado_tienda'],'pais_tienda'=>$data['pais_tienda']
                    ,'codigopin_tienda'=>$data['codigopin_tienda'],'business_licencia_numero'=>$data['business_licencia_numero']
                    ,'usd_moneda'=>$data['usd_moneda'],'pan_number'=>$data['pan_number']
                    ,'address_proof'=>$data['address_proof'],'address_proof_image'=>$imageName]);
                    
                }else{
                    //Actualizar tabla dedetalles business
                    VendorsBusinessDetail::insert(['vendor_id'=>Auth::guard('admin')->user()->vendor_id,'nombre_tienda'=>$data['nombre_tienda']
                    ,'celular_tienda'=>$data['celular_tienda'],'direccion_tienda'=>$data['direccion_tienda'],'ciudad_tienda'=>$data['ciudad_tienda'],'estado_tienda'=>$data['estado_tienda'],'pais_tienda'=>$data['pais_tienda']
                    ,'codigopin_tienda'=>$data['codigopin_tienda'],'business_licencia_numero'=>$data['business_licencia_numero']
                    ,'usd_moneda'=>$data['usd_moneda'],'pan_number'=>$data['pan_number']
                    ,'address_proof'=>$data['address_proof'],'address_proof_image'=>$imageName]);
